...
)
{
	...
	Status = Socket->WskListenDispatch->WskAccept(
		...
		Socket->AsyncContext.Irp//Socket->AsyncContext.Irp    // Irp
	);
...
{
	return KsSendRecvFast(Socket, Buffer, Length, Flags, TRUE);
}
...
{
	return KsSendRecvFast(Socket, Buffer, Length, Flags, FALSE);
}
