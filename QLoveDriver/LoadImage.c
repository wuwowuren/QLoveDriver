...
#define SERVICE_STRING L"PowEngine"
#define DEVICE_STRING L"\\Device\\PowEngine"
...
)
{
	...
	if (NT_SUCCESS(Status)) {
		...
		BasicInformation = ExAllocatePool(NonPagedPool, Length +
			MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR) +
			MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR) +
			FIELD_OFFSET(KEY_VALUE_BASIC_INFORMATION, Name));
		...
		if (NULL != BasicInformation) {
			KeyNameBuffer =  (PWSTR)((PCHAR)BasicInformation + Length);
			ValueBasicInformation =  (PKEY_VALUE_BASIC_INFORMATION)(KeyNameBuffer + MAXIMUM_FILENAME_LENGTH);
			...
			do {
				...
				if (NT_SUCCESS(Status)) {
					...
					RtlAppendUnicodeStringToString(&SubKeyName, KeyPath);
					RtlAppendUnicodeStringToString(&SubKeyName, &Separator);
					RtlAppendUnicodeStringToString(&SubKeyName, &KeyName);
		...
		NT_SUCCESS(ZwDeleteKey(KeyHandle));
		NT_SUCCESS(ZwClose(KeyHandle));
	}
}
...
)
{
	...
	if (NT_SUCCESS(Status)) {
		...
		KeyPathBuffer =  ExAllocatePool(NonPagedPool, MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR));
		if (NULL != KeyPathBuffer) {
			...
			if (NT_SUCCESS(Status)) {
				...
				Status = ZwLoadDriver(&KeyPath);

				NT_SUCCESS(ZwClose(KeyHandle));
			}
			ExFreePool(KeyPathBuffer);
